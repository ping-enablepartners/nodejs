...
class TokenManager {
    ...
    async prepareAuthParams (options) {
        const responseType = (options && options.responseType) ? options.responseType
            : (this.config.responseType ? this.config.responseType : ["token", "id_token"]);
        const scopes = (options && options.scopes) ? options.scopes
            : (this.config.scopes ? this.config.scopes : ["openid"]);
        const pkce = (options && options.pkce) ? options.pkce
            : (this.config.pkce !== undefined ? this.config.pkce : true);

        if (scopes.indexOf("openid") === -1) {
            scopes.push("openid");
        }
        const oauthParams = {
            useRefreshTokens: Boolean(this.config.useRefreshTokens),
            pkce: pkce,
    ...
    async handleAuthenticationResponse (authParams, authResponse, tokenEndpoint) {
        ...
        const pkce = Boolean(this.config.pkce);
        let response = {...authResponse};
        if (authResponse.code && pkce) {
    ...
    async parseRedirectUrl (options) {
        ...
        let paramStr;
        if (options.pkce || this.config.pkce) {
